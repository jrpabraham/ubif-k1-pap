...
required.packages <- c("dplyr", "multiwayvcov", "multcomp", "reshape2", "knitr", "Hmis", "tidyverse", "knitr", "broom")
...
k1_df <- read.delim(file = "K1__Field_Survey_v34+35_DeID.csv", sep = ",", header = FALSE, stringsAsFactors = FALSE, na.strings = "", skip = 2, nrows = 598, col.names = varnames) %>%
...
k1_df$vid.skills <- k1_df$vid.num > 0

# k1_df$vid.skills <- factor(as.character(k1_df$vid.num),
#                            levels=c("0", "1", "2"),
#                            labels=c("NoSkills", "Skills", "Skills"))
...
ms.vid.skills <- k1_df[,1:180] %>%  ## SELECT COLUMN I CARE ABOUT FIRST
  # dplyr::select(vid.skills, condition) %>% ## SELECT COLUMN I CARE ABOUT FIRST
  group_by(condition) %>%
  summarise(perc.vid = mean(vid.skills)) %>%
  ungroup()
